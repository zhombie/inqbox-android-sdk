...
import kz.qbox.sdk.domain.model.call.type.CallType
import kz.qbox.sdk.domain.model.content.ContentType
import kz.qbox.sdk.domain.model.content.asContentType
...
import kz.qbox.sdk.socket.emitter.JSONArrayListener
import kz.qbox.sdk.socket.emitter.JSONObjectListener
import kz.qbox.sdk.socket.event.Event
import kz.qbox.sdk.socket.listener.*
import kz.qbox.sdk.socket.logger.Logger
import kz.qbox.sdk.socket.model.*
import kz.qbox.sdk.socket.repository.SocketRepository
...
class SocketClient private constructor() : SocketRepository {
    ...
    private val language: String
        get() {
            val language = kz.qbox.sdk.socket.Socket.getLanguage()
            return language.key
        }
    ...
    override fun registerMessageEventListener(): Boolean =
        registerEventListener(Event.Incoming.MESSAGE, onMessageListener)

    override fun unregisterMessageEventListener(): Boolean =
        unregisterEventListener(Event.Incoming.MESSAGE, onMessageListener)

    override fun registerChatBotDashboardEventListener(): Boolean =
        registerEventListener(Event.Incoming.CATEGORY_LIST, onCategoryListListener)

    override fun unregisterChatBotDashboardEventListener(): Boolean =
        unregisterEventListener(Event.Incoming.CATEGORY_LIST, onCategoryListListener)

    override fun registerUsersQueueEventListener(): Boolean =
        registerEventListener(Event.Incoming.USER_QUEUE, onUserQueueListener)

    override fun unregisterUsersQueueEventListener(): Boolean =
        unregisterEventListener(Event.Incoming.USER_QUEUE, onUserQueueListener)

    override fun registerCallAgentGreetEventListener(): Boolean =
        registerEventListener(Event.Incoming.OPERATOR_GREET, onOperatorGreetListener)

    override fun unregisterCallAgentGreetEventListener(): Boolean =
        unregisterEventListener(Event.Incoming.OPERATOR_GREET, onOperatorGreetListener)

    override fun registerCallAgentTypingEventListener(): Boolean =
        registerEventListener(Event.Incoming.OPERATOR_TYPING, onOperatorTypingListener)

    override fun unregisterCallAgentTypingEventListener(): Boolean =
        unregisterEventListener(Event.Incoming.OPERATOR_TYPING, onOperatorTypingListener)

    override fun registerCard102UpdateEventListener(): Boolean =
        registerEventListener(Event.Incoming.CARD102_UPDATE, onCard102UpdateListener)

    override fun unregisterCard102UpdateEventListener(): Boolean =
        unregisterEventListener(Event.Incoming.CARD102_UPDATE, onCard102UpdateListener)

    override fun registerLocationUpdateEventListener(): Boolean =
        registerEventListener(Event.Incoming.LOCATION_UPDATE, onLocationUpdate)

    override fun unregisterLocationUpdateEventListener(): Boolean =
        unregisterEventListener(Event.Incoming.LOCATION_UPDATE, onLocationUpdate)

    override fun registerUserCallFeedbackEventListener(): Boolean =
        registerEventListener(Event.Incoming.FEEDBACK, onFeedbackListener)

    override fun unregisterUserCallFeedbackEventListener(): Boolean =
        unregisterEventListener(Event.Incoming.FEEDBACK, onFeedbackListener)

    override fun registerFormInitializeEventListener(): Boolean =
        registerEventListener(Event.Incoming.FORM_INIT, onFormInitListener)

    override fun unregisterFormInitializeEventListener(): Boolean =
        unregisterEventListener(Event.Incoming.FORM_INIT, onFormInitListener)

    override fun registerFormFinalizeEventListener(): Boolean =
        registerEventListener(Event.Incoming.FORM_FINAL, onFormFinalListener)

    override fun unregisterFormFinalizeEventListener(): Boolean =
        unregisterEventListener(Event.Incoming.FORM_FINAL, onFormFinalListener)

    override fun registerTaskMessageEventListener(): Boolean =
        registerEventListener(Event.Incoming.TASK_MESSAGE, onTaskMessageListener)

    override fun unregisterTaskMessageEventListener(): Boolean =
        unregisterEventListener(Event.Incoming.TASK_MESSAGE, onTaskMessageListener)
    ...
    override fun sendCallInitialization(callInitialization: CallInitialization) {
        ...
        call(Event.Outgoing.INITIALIZE, callInitialization)
    }
    ...
    override fun sendCallReinitialization(callInitialization: CallInitialization) {
        ...
        call(Event.Outgoing.REINITIALIZE, callInitialization)
    }
...
//            putIfValueNotNull("action", "redial")
    ...
    override fun getCategories(parentId: Long) {
        ...
        emit(Event.Outgoing.USER_DASHBOARD, jsonObject {
    ...
    override fun sendUserLanguage(language: Language) {
        ...
        emit(Event.Outgoing.USER_LANGUAGE, jsonObject {
    ...
    override fun sendUserTextMessage(message: String) {
        ...
        emit(Event.Outgoing.USER_MESSAGE, jsonObject {
    ...
    override fun sendUserCallFeedback(chatId: Long, rating: Int) {
        ...
        emit(Event.Outgoing.USER_FEEDBACK, jsonObject {
            put("r", rating)
            put("chat_id", chatId)
        }) {}
    }
    ...
    override fun sendFuzzyTaskConfirmation(name: String, email: String, phone: String) {
        emit(Event.Outgoing.CONFIRM_FUZZY_TASK, jsonObject {
            put("name", name)
            put("email", email)
            put("phone", phone)
            put("res", '1')
    ...
    override fun sendCancel() {
        emit(Event.Outgoing.CANCEL) {}
    }

    override fun sendPendingCallCancellation() {
        emit(Event.Outgoing.CANCEL_PENDING_CALL) {}
    }
    ...
    private val onFormInitListener by lazy {
        JSONObjectListener { jsonObject ->
            Logger.debug(TAG, "[${Event.Incoming.FORM_INIT}] jsonObject: $jsonObject")
    ...
    private val onFormFinalListener by lazy {
        JSONObjectListener { jsonObject ->
            Logger.debug(TAG, "[${Event.Incoming.FORM_FINAL}] jsonObject: $jsonObject")
    ...
    private val onOperatorGreetListener by lazy {
        JSONObjectListener { jsonObject ->
            Logger.debug(TAG, "[${Event.Incoming.OPERATOR_GREET}] jsonObject: $jsonObject")
...
//            val name = jsonObject.optString("name")
    ...
    private val onOperatorTypingListener by lazy {
        Emitter.Listener {
            Logger.debug(TAG, "event [${Event.Incoming.OPERATOR_TYPING}]")
        }
    }

    private val onFeedbackListener by lazy {
        JSONObjectListener { jsonObject ->
            Logger.debug(TAG, "[${Event.Incoming.FEEDBACK}] jsonObject: $jsonObject")
...
//            val channel = jsonObject.getInt("channel")
    ...
    private val onMessageListener by lazy {
        JSONObjectListener { jsonObject ->
            Logger.debug(TAG, "[${Event.Incoming.MESSAGE}] jsonObject: $jsonObject")
...
//            val from = jsonObject.getStringOrNull("from")
    ...
    private val onCategoryListListener by lazy {
        JSONObjectListener { jsonObject ->
            Logger.debug(TAG, "[${Event.Incoming.CATEGORY_LIST}] jsonObject: $jsonObject")
    ...
    private val onCard102UpdateListener by lazy {
        JSONObjectListener { jsonObject ->
            Logger.debug(TAG, "[${Event.Incoming.CARD102_UPDATE}] jsonObject: $jsonObject")
    ...
    private val onLocationUpdate by lazy {
        JSONArrayListener { jsonArray ->
            ...
            Logger.debug(TAG, "[${Event.Incoming.LOCATION_UPDATE}] jsonArray: $jsonArray")
    ...
    private val onTaskMessageListener by lazy {
        JSONObjectListener { jsonObject ->
            Logger.debug(TAG, "[${Event.Incoming.TASK_MESSAGE}] jsonObject: $jsonObject")
