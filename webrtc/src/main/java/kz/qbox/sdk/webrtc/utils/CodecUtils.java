...
public class CodecUtils {
    ...
    public static String setStartBitrate(String codec, boolean isVideoCodec, String sdpDescription, int bitrateKbps) {
        ...
        if (codecRtpMap == null) {
            Logger.warn(TAG, "No rtpmap for " + codec + " codec");
            return sdpDescription;
        }
        Logger.debug(TAG, "Found " + codec + " rtpmap " + codecRtpMap + " at " + lines[rtpmapLineIndex]);
        ...
        for (int i = 0; i < lines.length; i++) {
            Matcher codecMatcher = codecPattern.matcher(lines[i]);
            if (codecMatcher.matches()) {
                Logger.debug(TAG, "Found " + codec + " " + lines[i]);
                if (isVideoCodec) {
                    lines[i] += "; " + VIDEO_CODEC_PARAM_START_BITRATE + "=" + bitrateKbps;
                } else {
                    lines[i] += "; " + AUDIO_CODEC_PARAM_BITRATE + "=" + (bitrateKbps * 1000);
                }
                Logger.debug(TAG, "Update remote SDP line: " + lines[i]);
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (!sdpFormatUpdated && i == rtpmapLineIndex) {
                ...
                Logger.debug(TAG, "Add remote SDP line: " + bitrateSet);
    ...
    public static String preferCodec(String sdpDescription, String codec, boolean isAudio) {
        ...
        if (mLineIndex == -1) {
            Logger.warn(TAG, "No mediaDescription line, so can't prefer " + codec);
            return sdpDescription;
        }
        ...
        if (codecPayloadTypes.isEmpty()) {
            Logger.warn(TAG, "No payload types with name " + codec);
            return sdpDescription;
        }
        ...
        Logger.debug(TAG, "Change media description from: " + lines[mLineIndex] + " to " + newMLine);
    ...
    private static @Nullable String movePayloadTypesToFront(List<String> preferredPayloadTypes, String mLine) {
        ...
        if (origLineParts.size() <= 3) {
            Logger.error(TAG, "Wrong SDP media description format: " + mLine);
            return null;
        }
