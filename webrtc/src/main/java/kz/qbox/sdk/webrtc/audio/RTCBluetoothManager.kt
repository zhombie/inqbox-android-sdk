...
package kz.qbox.sdk.webrtc.audio
...
import kz.qbox.sdk.webrtc.utils.ThreadUtils.currentThread
...
internal open class RTCBluetoothManager protected constructor(
    ...
    companion object {
        ...
        fun create(context: Context, audioManager: RTCAudioManager): RTCBluetoothManager {
            Logger.debug(TAG, "create: $currentThread")
            ...
            if (action == BluetoothHeadset.ACTION_CONNECTION_STATE_CHANGED) {
                ...
                if (state == BluetoothHeadset.STATE_AUDIO_CONNECTED) {
                    ...
                    if (bluetoothState == State.SCO_CONNECTING) {
                        ...
                        Logger.warn(TAG, "Unexpected state BluetoothHeadset.STATE_AUDIO_CONNECTED")
                    }
    ...
    fun start() {
        ...
        if (!hasPermission(context, Manifest.permission.BLUETOOTH)) {
            Logger.warn(TAG, "Process (pid=" + Process.myPid() + ") lacks BLUETOOTH permission")
            return
        }
        if (bluetoothState != State.UNINITIALIZED) {
            Logger.warn(TAG, "Invalid BT state")
            return
        }
        bluetoothHeadset = null
        bluetoothDevice = null
        ...
        if (bluetoothAdapter == null) {
            Logger.warn(TAG, "Device does not support Bluetooth")
            return
        }
...
//        if (bluetoothState == State.UNINITIALIZED) return
    ...
    private fun bluetoothTimeout() {
        ...
        if (scoConnected) {
            ...
            Logger.warn(TAG, "BT failed to connect after timeout")
            stopScoAudio()
        }
        updateAudioDeviceState()
